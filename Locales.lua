...
local S = {
    ...
    mbTypeDuration      = { "Buff持续时间",     "Buff Duration" },
    ...
    mbMaxDuration       = { "最大持续时间（秒）", "Max Duration (seconds)" },
    mbMaxDurationAuto   = { "设为 0 则自动从 buff 检测", "Set to 0 to auto-detect from buff" },
    ...
    mbShowTextDuration  = { "显示剩余时间",     "Show Remaining Time" },
    ...
    mbCatalogAuras      = { "Buffs（点击添加堆叠/持续时间条）", "Buffs (click to add stack/duration bar)" },
    mbCatalogEmpty      = { "未扫描到技能，请先脱战", "No spells found, leave combat first" },
    mbBarTexture        = { "条材质",           "Bar Texture" },
    mbCDMHint           = { "提示：Buff监控需要对应Buff已在冷却管理器中追踪，否则不生效", "Note: buff stack bars require the buff to be in the Cooldown Manager" },
    ...
    mbColorThreshold2   = { "染色阈值 2",       "Color Threshold 2" },
    mbColorThresholdTip2= { "0 = 关闭；大于等于此层数时使用阈值颜色 2", "0 = off; use threshold color 2 above this count" },
    mbThresholdColor2   = { "阈值颜色 2",       "Threshold Color 2" },
    ...
    mbCondActiveOnly    = { "仅激活时显示（仅对Buff持续时间生效）",     "Only When Active（Duration Only）" },
    ...
    bgOverrideSize      = { "覆盖图标尺寸",     "Override Icon Size" },
    bgOverrideSizeTip   = { "启用后，此分组的图标尺寸独立于主 Buff 组", "When enabled, this group's icon size is independent of the main Buff group" },
    bgIconWidth         = { "图标宽度",         "Icon Width" },
    bgIconHeight        = { "图标高度",         "Icon Height" },
